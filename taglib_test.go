...
func TestWriteTagLibAAC(t *testing.T) {
	fileName := "test.m4a"
	file, err := Read(fileName)
	defer file.Close()
	if err != nil {
	...
	tempDir, err := ioutil.TempDir("", "go-taglib-test-AAC")
	if err != nil {
		t.Fatalf("Cannot create temporary file for writing tests: %s", err)
	}

	tempFileName := path.Join(tempDir, "go-taglib-test.m4a")
	defer os.RemoveAll(tempDir)
...
func TestWriteTagLibFLAC(t *testing.T) {
	...
	tempDir, err := ioutil.TempDir("", "go-taglib-test-FLAC")
	if err != nil {
		t.Fatalf("Cannot create temporary file for writing tests: %s", err)
	}

	tempFileName := path.Join(tempDir, "go-taglib-test.flac")
	defer os.RemoveAll(tempDir)
